...
import fr.marc_nguyen.sensitivity.core.utils.div
import fr.marc_nguyen.sensitivity.core.utils.getEnvironmentalHdrSphericalHarmonics
...
class LightRenderer(context: Context, private val filament: Filament) {
    ...
    fun doFrame(frame: Frame) {
        ...
        with(frame.lightEstimate.environmentalHdrMainLightIntensity) {
            ...
            val rgbMax = kotlin.math.max(kotlin.math.max(get(0), get(1)), get(2))
            // prevent div by zero
            val color = V3(this).div(kotlin.math.max(0.00001f, rgbMax))
